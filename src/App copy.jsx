...
export const encodeStateToURL = (state) => {
  ...
  const list = state.tiles.map((t) => t.channel.trim()).filter(Boolean);
  if (list.length) params.set("streams", list.join(","));
  params.set("count", String(Math.min(state.activeCount, getLayoutMeta(state.layout).maxTiles)));
  const chatMask = state.tiles.map((t) => (t.showChat ? "1" : "0")).join("");
  if (chatMask.includes("1")) params.set("chat", chatMask);
...
function useRestoredState() {
  const [state, setState] = useState(() => {
    ...
    const def = { ...DEFAULT_STATE, layout };
    const layoutMeta = LAYOUTS[layout] || LAYOUTS["2x1"];
    let activeCount = Number(q.get("count"));
    if (!activeCount || activeCount < layoutMeta.minTiles || activeCount > layoutMeta.maxTiles) {
      activeCount = layoutMeta.maxTiles === 4 ? 4 : 2;
    }
    const streams = (q.get("streams") || "").split(",").filter(Boolean);
    const chatMask = (q.get("chat") || "").padEnd(4, "0");
    const tiles = def.tiles.map((t, i) => ({
      ...t,
      channel: streams[i] || "",
      showChat: chatMask[i] === "1" || (chatMask === "" ? true : t.showChat),
    }));
...
export default function App() {
  ...
  const visibleCount = React.useMemo(() => Math.min(appState.activeCount, meta.maxTiles), [appState.activeCount, meta.maxTiles]);
...
function Grid({ layout, activeCount, tiles, parentDomain, onChangeChannel, onToggleChat, onFullscreen }) {
  ...
  const gridTiles = [
    ...cells,
    ...Array.from({ length: totalCells - cells.length }, (_, i) => null),
  ];
      ...
      <div className="absolute top-0 left-0 w-full flex items-center gap-2 px-3 py-2 bg-black/60 backdrop-blur-sm z-10 opacity-0 group-hover:opacity-100 transition">
        ...
        <button
          className={`px-2 py-1 rounded border text-xs font-semibold ${
            tile.showChat
              ? "bg-[#7c3aed]/90 border-[#7c3aed] text-white"
